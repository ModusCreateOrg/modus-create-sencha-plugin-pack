...
Ext.define('Ext.util.Observable', {
        ...
        if (me.eventsSuspended === true) {
            ...
            if (event.bubble) {
                if (event.fire.apply(event, args.slice(1)) === false) {
                    ...
                    parent = me.getBubbleParent(me);
                    while (parent) {
                        if (parent.fireEvent.apply(parent, args) === false) {
                            return false;
                        }
                        parent = me.getBubbleParent(parent);
                    }
                }
            } else {
                args.shift();
     ...
     * @private
     * @param {Ext.util.Observable} observable The observable to get the bubble target for.
     * @return {Ext.util.Observable} The bubble parent. null is returned if no bubble target exists
     */
    getBubbleParent: function(observable){
        var parent = observable.getBubbleTarget && observable.getBubbleTarget();
